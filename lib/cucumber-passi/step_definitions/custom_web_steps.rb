...
When /^I (follow|press) "([^"]+)" for #{requested_model_pattern}$/ do |action, field, model_name, record_identifier|
  ...
  When %{I #{action} "#{field}" within "#{model_selector}"}
end
...
Then /^I should be editing (.+)$/ do |page_name|
  Then %(I should be on the "edit" page for #{page_name})
end
...
Then /^I should see the following:$/ do |table|
  ...
  table.rows_hash.each do |field, value|
    ...
    Then %(I should see "#{value}" within "##{element_id}")
